...
final class SessionDelegate: NSObject {
    ...
    var didOpenWebSocket: (() -> Void)?
    var didCloseWebSocket: (() -> Void)?
...
//    }
    ...
    func urlSession(_ session: URLSession, task: URLSessionTask, didSendBodyData bytesSent: Int64, totalBytesSent: Int64, totalBytesExpectedToSend: Int64) {
        NSLog("URLSession: \(session), task: \(task), didSendBodyData: \(bytesSent), totalBytesSent: \(totalBytesSent), totalBytesExpectedToSent: \(totalBytesExpectedToSend), progress: \(task.progress)")
    }
...
extension SessionDelegate: URLSessionWebSocketDelegate {
    ...
    func urlSession(_ session: URLSession, webSocketTask: URLSessionWebSocketTask, didCloseWith closeCode: URLSessionWebSocketTask.CloseCode, reason: Data?) {
        NSLog("URLSession: \(session), webSocketTask: \(webSocketTask), didCloseWith: \(closeCode), reason: \(reason.map { String(decoding: $0, as: UTF8.self) } ?? "None")")
        didCloseWebSocket?()
    }
}
